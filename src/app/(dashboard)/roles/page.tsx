...
export default function RolesPage() {
  ...
  const dynamicRolesWithCounts = dynamicRoles
    .filter((role: any) => {
      ...
      if (baseRoleKeys.has(normalized)) return false;

      // Evita duplicados por diferencias de mayúsculas/minúsculas en la tabla roles.
      if (seenDynamicRoleKeys.has(normalized)) return false;
    ...
    .map((role: any, index: number) => {
      const roleName = String(role.name).trim();
      return {
        id: roleName,  // Coincide con role_permissions.role
        name: roleName,
